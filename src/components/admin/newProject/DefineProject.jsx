      ...
      <Flex direction="column" maxW="680px">
            ...
            <MenuList>
                {
                    projectType && projectType.map((val,ind)=> 
                      <MenuItem key={ind} onClick={() => {setFormData({...formData,projectType: val.name});setTemplateState({...templateState,template_type_id: val._id})}}>{val.name}</MenuItem>)
                }
            </MenuList>
          </Menu>
        </Flex>
        ...
        <Flex align="center" mb="20px">
          ...
          <Menu >
            ...
            <MenuList>
                {
                    segment && segment.map((val,ind)=> <MenuItem key={ind} onClick={() => {setFormData({...formData,segment: val.name}); setTemplateState({...templateState,template_segment_id: val._id})}}>{val.name}</MenuItem>)
                }
            </MenuList>
          </Menu>
        </Flex>
        ...
        <Flex align="center" mb="20px">
          ...
          <Menu >
            ...
            <MenuList>
                {
                    industry && industry.map((val,ind)=> <MenuItem key={ind} onClick={() => {setFormData({...formData,industry: val.name}); setTemplateState({...templateState,template_industry_id: val._id})}}>{val.name}</MenuItem>)
                }
            </MenuList>
          </Menu>
        </Flex>
        ...
        <Flex align="center" mb="20px">
          ...
          <Textarea w="80%" placeholder="Enter the use case" value={formData.useCase} onChange={(e) => {setFormData({...formData,useCase: e.target.value}); setTemplateState({...templateState,template_usecase: e.target.value})}}/>
        </Flex>
      </Flex>
  );
};
